/**
 * Object representing the configuration for fetching completion of training requirements for users.
 * Starting at 2 months out, then aggregating by month.
 *
 * @constant {Object} trainingCompletionByMonth
...
export async function trainingCompletionByMonth() {
    const payload = {
        resource: 'user',
        fields: [
            'last_name',
            'required_training',
            'training_completed',
            'training_date_completed',
            'training_date_due'
        ],
        filters: [],
        joins: [],
        sort_by: 'training_date_due',
        sort_order: 'asc',
        format: 'json',
        page: 1,
        page_size: 30,
